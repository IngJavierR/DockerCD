pipeline {
    ...
    stages {
        ...
        stage('Expresso test') {
            steps {
                DOCKERID = sh 'docker run --privileged -d -p 6080:6080 -p 5554:5554 -p 5555:5555 -e DEVICE=\'Samsung Galaxy S6\' butomo1989/docker-android-x86-7.1.1 > dockerid'
    ...
    post { 
        always { 
            deleteDir()
            sh 'docker rm -f' + readFile('commandResult').trim()
        }
        success {
            echo 'I succeeeded!'
        }
